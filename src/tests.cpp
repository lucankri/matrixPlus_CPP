...
TEST(Constructor_param, test_6) { ASSERT_ANY_THROW(S21Matrix M(0, 5)); }

TEST(Constructor_param, test_7) {
  S21Matrix M(0, 0);
...
TEST(Constructor_move, test_1) {
  ...
  ASSERT_EQ(M2.GetRows(), 6);
  ASSERT_EQ(M2.GetCols(), 6);
  ASSERT_EQ(M.GetRows(), 0);
  ASSERT_EQ(M.GetCols(), 0);
}
...
TEST(Method_Determinant, test_4) {
  S21Matrix M(50, 50);
  S21Matrix M2(50, 50);
  random_device rd;
  mt19937 gen(rd());
  ...
  ASSERT_DOUBLE_EQ(res, res2);
}

TEST(Method_Determinant, test_5) {
  S21Matrix M(4, 4);
  M(0, 0) = 0;
  M(0, 1) = 56;
  M(0, 2) = 4;
  ...
  M(3, 3) = 678;
  double res = M.Determinant();
  ASSERT_DOUBLE_EQ(res, -26770223.207819924);
}
